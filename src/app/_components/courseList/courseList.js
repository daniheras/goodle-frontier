...
class CourseList extends Component {
    ...
    handleRegisterCourse() {
        axios.post($api_URL + '/addusercourse', {
            user_id: JSON.parse(sessionStorage.getItem('user')).id,
            course_id: this.state.redirectId
        })
            .then(data => {
                console.log(data);
                axios.get($api_URL + '/courses/' + JSON.parse(sessionStorage.getItem('user')).id)
                ...
                courses.push(
                    <div key={key}>
                        ...
                        <Button color="primary"
                                onClick={(e) => this.showModal(course.id, course.name, 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do ')}>View</Button>
                    </div>);
            });
            if (courses.length === 0) {
        ...
        return (
            <div>
                ...
                <Modal isOpen={this.state.visible} toggle={this.handleCancel} className={this.props.className}>
                    ...
                    <ModalFooter>
                        {this.state.type === 'myCourses' ?
                            <Button color="primary" onClick={this.handleOk}>Enter Course</Button> :
                            <Button color="primary" onClick={this.handleRegisterCourse}>Join Course</Button>}
